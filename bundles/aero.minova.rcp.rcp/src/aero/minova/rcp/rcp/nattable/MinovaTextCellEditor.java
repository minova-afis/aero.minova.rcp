...
public class MinovaTextCellEditor extends AbstractCellEditor {
	 ...
	 * via convenience methods of this TextCellEditor.
	 */
	...
	protected Control activateCell(final Composite parent, Object originalCanonicalValue) {
		...
		if (this.inputValidationErrorHandler instanceof RenderErrorHandling) {
			IStyle validationErrorStyle = this.configRegistry.getConfigAttribute(
					EditConfigAttributes.VALIDATION_ERROR_STYLE, DisplayMode.EDIT, this.labelStack.getLabels());
		...
		textControl.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent event) {
				if (isCommitOnEnter() && (event.keyCode == SWT.CR || event.keyCode == SWT.KEYPAD_CR)) {
					...
					if (MinovaTextCellEditor.this.moveSelectionOnEnter
					...
					} else if (MinovaTextCellEditor.this.commitOnLeftRight && control.getSelectionCount() == 0
	 ...
	 * {@link MinovaTextCellEditor#getSelectionMode()}.
	 *
	 ...
	 * @since 1.4
	 */
	public void enableContentProposal(IControlContentAdapter controlContentAdapter,
