...
public class MinovaCustomContentProposalAdapter extends ContentProposalAdapter {

	class ContentProposalPopup extends PopupDialog {
		/*
		 * The listener we install on the popup and related controls to determine when to close the popup. Some events (move, resize, close, deactivate) trigger
		 * closure as soon as they are received, simply because one of the registered listeners received them. Other events depend on additional circumstances.
		 */
		...
		private final class TargetControlListener implements Listener {
			...
			public void handleEvent(Event e) {
				...
				if (key == 0) {
					...
					default:
						if (e.keyCode != SWT.CAPS_LOCK && e.keyCode != SWT.NUM_LOCK && e.keyCode != SWT.MOD1 && e.keyCode != SWT.MOD2 && e.keyCode != SWT.MOD3
								&& e.keyCode != SWT.MOD4) {
							close();
						}
						return;
		...
		/**
		 * Constructs a new instance of this popup, specifying the control for which this popup is showing content, and how the proposals should be obtained and
		 * displayed.
		 *
		 * @param infoText
		...
		/*
		 * Creates the content area for the proposal popup. This creates a table and places it inside the composite. The table will contain a list of all the
		 * proposals.
		...
		protected void adjustBounds() {
			...
			if (constrainedBounds.y < initialY) getShell().setBounds(initialX, location.y - popupSize.y, popupSize.x, popupSize.y);
		...
		/*
		 * Return an empty array. Used so that something always shows in the proposal popup, even if no proposal provider was specified.
		 */
		...
		/**
		 * Opens this ContentProposalPopup. This method is extended in order to add the control listener when the popup is opened and to invoke the secondary
		 * popup if applicable.
		 *
		...
		/*
		 * Request the proposals from the proposal provider, and recompute any caches. Repopulate the popup if it is open.
		 */
		private void recomputeProposals(String filterText) {
			IContentProposal[] allProposals = getProposals();
			if (allProposals == null) allProposals = getEmptyProposalArray();
		...
		/*
		 * In an async block, request the proposals. This is used when clients are in the middle of processing an event that affects the widget content. By
		 * using an async, we ensure that the widget content is up to date with the event.
		 */
	...
	/**
	 * Indicates that the contents of the control should not be modified when a proposal is chosen. This is typically used when a client needs more specialized
	 * behavior when a proposal is chosen. In this case, clients typically register an IContentProposalListener so that they are notified when a proposal is
	 * chosen.
	 */
	...
	/**
	 * Indicates that a cumulative filter applies as keys are typed in the popup. That is, each character typed will be added to the filter.
	 *
	 * @deprecated As of 3.4, filtering that is sensitive to changes in the control content should be performed by the supplied
	 *             {@link IContentProposalProvider}, such as that performed by {@link SimpleContentProposalProvider}
	 */
	@Deprecated
	...
	/*
	 * Integer that indicates how an accepted proposal should affect the control. One of PROPOSAL_IGNORE, PROPOSAL_INSERT, or PROPOSAL_REPLACE. Default value is
	 * PROPOSAL_INSERT.
	 */
	private int proposalAcceptanceStyle = PROPOSAL_INSERT;

	/*
	 * A boolean that indicates whether key events received while the proposal popup is open should also be propagated to the control. Default value is true.
	 */
	...
	/*
	 * Flag that indicates whether the adapter is enabled. In some cases, adapters may be installed but depend upon outside state.
	 */
	...
	/*
	 * A boolean indicating whether a keystroke has been received. Used to see if an autoactivation delay was interrupted by a keystroke.
	 */
	...
	/*
	 * The remembered position of the insertion position. Not all controls will restore the insertion position if the proposal popup gets focus, so we need to
	 * remember it.
	 */
	private int insertionPos = -1;

	/*
	 * The remembered selection range. Not all controls will restore the selection position if the proposal popup gets focus, so we need to remember it.
	 */
	 ...
	 */
	public MinovaCustomContentProposalAdapter(Control control,
	...
	/**
	 * Set the label provider that is used to show proposals. The lifecycle of the specified label provider is not managed by this adapter. Clients must dispose
	...
	/**
	 * Return the proposal provider that provides content proposals given the current content of the field. A value of <code>null</code> indicates that there
	 * are no content proposals available for the field.
	 *
	 ...
	 *
	 * @return An array of characters that trigger auto-activation of content proposal. If specified, these characters will trigger auto-activation of the
	 ...
	 *         specified keyStroke will invoke content proposal. If this value is <code>null</code> and the keyStroke value is <code>null</code>, then all
	 ...
	 *            proposal popup, regardless of whether an explicit invocation keyStroke was specified. If this parameter is <code>null</code>, then only a
	 *            specified keyStroke will invoke content proposal. If this parameter is <code>null</code> and the keyStroke value is <code>null</code>, then
	 ...
	 *
	 * @return a constant indicating how an accepted proposal should affect the control's content. Should be one of <code>PROPOSAL_INSERT</code>,
	 *         <code>PROPOSAL_REPLACE</code>, or <code>PROPOSAL_IGNORE</code>. (Default is <code>PROPOSAL_INSERT</code>).
	 */
	 ...
	 *
	 * @param acceptance
	 *            a constant indicating how an accepted proposal should affect the control's content. Should be one of <code>PROPOSAL_INSERT</code>,
	 ...
	 *
	 * @return a constant indicating how keystrokes in the proposal popup affect filtering of the proposals shown. <code>FILTER_NONE</code> specifies that no
	 *         filtering will occur in the content proposal list as keys are typed. <code>FILTER_CHARACTER</code> specifies the content of the popup will be
	 *         filtered by the most recently typed character. <code>FILTER_CUMULATIVE</code> is deprecated and no longer recommended. It specifies that the
	...
	/**
	 * Set the integer style that indicates how keystrokes affect the content of the proposal popup while it is open. Popup-based filtering is useful for
	 * narrowing and navigating the list of proposals provided once the popup is open. Filtering of the proposals will occur even when the control content is
	 * not affected by user typing. Note that automatic filtering is not used to achieve content-sensitive filtering such as auto-completion. Filtering that is
	 * sensitive to changes in the control content should be performed by the supplied {@link IContentProposalProvider}.
	 *
	 * @param filterStyle
	 *            a constant indicating how keystrokes received in the proposal popup affect filtering of the proposals shown. <code>FILTER_NONE</code>
	 *            specifies that no automatic filtering of the content proposal list will occur as keys are typed in the popup. <code>FILTER_CHARACTER</code>
	 *            specifies that the content of the popup will be filtered by the most recently typed character. <code>FILTER_CUMULATIVE</code> is deprecated
	 ...
	 *
	 * @return a boolean that indicates whether key events (including auto-activation characters) should be propagated to the adapted control when the proposal
	 *         popup is open. Default value is <code>true</code>.
	 */
	...
	/**
	 * Return the content adapter that can get or retrieve the text contents from the adapter's control. This method is used when a client, such as a content
	 * proposal listener, needs to update the control's contents manually.
	 *
	 ...
	 *
	 * @param enabled
	 *            <code>true</code> if the adapter is enabled and responding to user input, <code>false</code> if it is ignoring user input.
	 */
	 ...
	 *
	 * @param listener
	 *            the IContentProposalListener to be added as a listener. Must not be <code>null</code>. If an attempt is made to register an instance which is
	 *            already registered with this instance, this method has no effect.
	 ...
	 *
	 * @param listener
	 *            the IContentProposalListener to be removed as a listener. Must not be <code>null</code>. If the listener has not already been registered, this
	 ...
	 *
	 * @param listener
	 *            the IContentProposalListener2 to be added as a listener. Must not be <code>null</code>. If an attempt is made to register an instance which is
	 *            already registered with this instance, this method has no effect.
	 * @since 3.3
	 ...
	 *
	 * @param listener
	 *            the IContentProposalListener2 to be removed as a listener. Must not be <code>null</code>. If the listener has not already been registered,
			...
			public void handleEvent(Event e) {
				...
				case SWT.KeyDown:
					...
					if (popup != null) {
						...
						if (propagateKeys && e.character != 0) watchModify = true;
					...
					/*
					 * The triggering keystroke was not invoked. If a character was typed, compare it to the autoactivation characters.
					 */
					if (e.character != 0) {
						if (autoActivateString != null) {
					...
					if (allowsAutoActivate() && watchModify) {
						...
						if (isControlContentEmpty()) {
							...
							if (autoActivateString == null) {
								...
								if (!shouldPopupRemainOpen()) closeProposalPopup();
							}
							}
						}
					break;
	...
	/**
	 * Open the proposal popup and display the proposals provided by the proposal provider. If there are no proposals to be shown, do not show the popup. This
	 * method returns immediately. That is, it does not wait for the popup to open or a proposal to be selected.
	 *
	 * @param autoActivated
	 *            a boolean indicating whether the popup was autoactivated. If false, a beep will sound when no proposals can be shown.
	 */
	...
	/**
	 * Open the proposal popup and display the proposals provided by the proposal provider. This method returns immediately. That is, it does not wait for a
	 * proposal to be selected. This method is used by subclasses to explicitly invoke the opening of the popup. If there are no proposals to show, the popup
	...
	/**
	 * Close the proposal popup without accepting a proposal. This method returns immediately, and has no effect if the proposal popup was not open. This method
	 * is used by subclasses to explicitly close the popup based on additional logic.
	 *
	 * @since 3.3
	 */
	...
	/*
	 * Set the text content of the control to the specified text, setting the cursorPosition at the desired location within the new contents.
	 */
	...
	/*
	 * Insert the specified text into the control content, setting the cursorPosition at the desired location within the new contents.
	 */
	...
	/*
	 * Return whether a proposal popup should remain open. If it was autoactivated by specific characters, and none of those characters remain, then it should
	 * not remain open. This method should not be used to determine whether autoactivation has occurred or should occur, only whether the circumstances would
	 * dictate that a popup remain open.
	 */
	private boolean shouldPopupRemainOpen() {
		// If we always autoactivate or never autoactivate, it should remain open
		if (autoActivateString == null || autoActivateString.isEmpty()) return true;
		String content = getControlContentAdapter().getControlContents(getControl());
		for (int i = 0; i < autoActivateString.length(); i++) {
			if (content.indexOf(autoActivateString.charAt(i)) >= 0) return true;
		}
		return false;
	}
	...
	/**
	 * Sets focus to the proposal popup. If the proposal popup is not opened, this method is ignored. If the secondary popup has focus, focus is returned to the
	 * main proposal popup.
	 *
	 * @since 3.6
	 */
	public void setProposalPopupFocus() {
		if (isValid() && popup != null) popup.getShell().setFocus();
	}
	...
	public boolean isProposalPopupOpen() {
		if (isValid() && popup != null) return true;
		return false;
	}
