...
public class EPerspectiveSwitcher {
	...
	private EventHandler toBeRenderedHandler = new EventHandler() {
		...
		public void handleEvent(Event event) {
			...
			MPerspective perspective = (MPerspective) changedElement;
			if (!perspective.getParent().isToBeRendered()) return;

			MWindow perspectiveWindow = perspective.getContext().get(MWindow.class);
			if (window != perspectiveWindow) return;

			if (perspective.isToBeRendered()) control.addPerspectiveShortcut(perspective);
			else control.removePerspectiveShortcut(perspective);
		}
	};
