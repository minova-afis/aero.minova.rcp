...
public class PreferenceWindowModel {
	...
	private PreferenceTabDescriptor buildAnwendungsTab() {
		...
		ptd.add(psd);
		PreferenceDescriptor pd = new PreferenceDescriptor("licenceWarning", "LizenzWarnung [wochen]", 0.1,
	...
	private PreferenceTabDescriptor buildDarstellungsTab() {
		...
		psd.add(pd);
		pd = new PreferenceDescriptor("timezone", "Zeitzone", 0.3, DisplayType.ZONEID, TimeZone.getDefault().toString(),
				CustomTimeZone.getTimeZones(locale).toArray());
		psd.add(pd);

		psd = new PreferenceSectionDescriptor("designpreferences", "Design-Einstellungen", 0.2);
		ptd.add(psd);
		pd = new PreferenceDescriptor("font", "Schriftgröße", 0.1, DisplayType.COMBO, "M", "S", "M", "L", "XL");
		psd.add(pd);
		pd = new PreferenceDescriptor("symbolMenu", "Symbole(Menü, Details)", 0.2, DisplayType.COMBO, "24x24", "16x16",
				"24x24", "32x32", "48x48", "64x64");
		psd.add(pd);
		pd = new PreferenceDescriptor("symbolToolbar", "Symbole (Toolbar)", 0.3, DisplayType.COMBO, "32x32", "16x16",
				"24x24", "32x32", "48x48", "64x64");
		psd.add(pd);
		return ptd;
	}
	...
	private PreferenceTabDescriptor buildErweiterungTab() {
		...
		ptd.add(psd);
		pd = new PreferenceDescriptor("masks", "Masken mehrfach öffnen", 0.1, DisplayType.CHECK, false);
		psd.add(pd);
		pd = new PreferenceDescriptor("dragdrop", "DragDrop deaktivieren", 0.2, DisplayType.CHECK, false);
		psd.add(pd);
		pd = new PreferenceDescriptor("icons", "Alle Icons in Symbolleiste einblenden ", 0.3, DisplayType.CHECK, false);
		psd.add(pd);
		...
		pd = new PreferenceDescriptor("indexautoupdate", "Index automatisch nach dem Speichern aktualisieren", 0.5,
				DisplayType.CHECK, false);
		psd.add(pd);
		pd = new PreferenceDescriptor("reportwindow", "Meldungsfenster an Menüleiste", 0.6, DisplayType.CHECK, true);
		psd.add(pd);
		pd = new PreferenceDescriptor("descriptionButton", "Beschreibung für Schaltflächen einblenden", 0.7,
				DisplayType.CHECK, true);
		psd.add(pd);
		pd = new PreferenceDescriptor("maskbuffer", "Masken Puffer benutzen", 0.8, DisplayType.CHECK, true);
		psd.add(pd);
		psd = new PreferenceSectionDescriptor("buffer", "Puffer", 0.2);
		ptd.add(psd);
		pd = new PreferenceDescriptor("displaybuffer", "Anzeige Puffer [ms]", 0.1, DisplayType.INTEGER, 20);
		psd.add(pd);
		pd = new PreferenceDescriptor("maxbuffer", "Max. Puffer [ms]", 0.2, DisplayType.INTEGER, 90);
		psd.add(pd);

		psd = new PreferenceSectionDescriptor("table", "Tabelle", 0.3);
		ptd.add(psd);
		pd = new PreferenceDescriptor("selectiondelay", "Auswahltverzögerung [ms]", 0.1, DisplayType.INTEGER, 150);
		psd.add(pd);

		psd = new PreferenceSectionDescriptor("lookup", "Nachschlagen", 0.5);
		ptd.add(psd);
		pd = new PreferenceDescriptor("sizeautoadjust", "Größe automatisch anpassen", 0.1, DisplayType.CHECK, false);
		psd.add(pd);
		...
		pd = new PreferenceDescriptor("buttondetailarea", "Schaltfläche im Detailbereich", 0.2, DisplayType.CHECK,
				true);
		psd.add(pd);
		pd = new PreferenceDescriptor("showlookups", "Zeige Nachschläge", 0.3, DisplayType.CHECK, true);
		psd.add(pd);
		pd = new PreferenceDescriptor("fadeingroups", "Gruppen einblenden", 0.4, DisplayType.CHECK, true);
		psd.add(pd);
		pd = new PreferenceDescriptor("showchangedrow", "Zeige geänderte Zeilen", 0.5, DisplayType.CHECK, true);
		psd.add(pd);
	...
	private PreferenceTabDescriptor buildDruckenTab() {
		...
		ptd.add(psd);
		pd = new PreferenceDescriptor("xmlxsdcreate", "XML + XDS erstellen", 0.1, DisplayType.CHECK, false);
		psd.add(pd);
		pd = new PreferenceDescriptor("fontChooser", "Schriftart Inhaltsverzeichnis", 0.2, DisplayType.FONT, null);
		psd.add(pd);
		pd = new PreferenceDescriptor("optimizewidth", "Breiten optimieren", 0.3, DisplayType.CHECK, true);
		psd.add(pd);
		pd = new PreferenceDescriptor("hideemptycolumn", "Leere Spalten verbergen", 0.4, DisplayType.CHECK, true);
		psd.add(pd);
		pd = new PreferenceDescriptor("hidegoupcolumns", "Gruppenspalten verbergen", 0.5, DisplayType.CHECK, true);
		psd.add(pd);
		pd = new PreferenceDescriptor("hidesearchdetails", "Suchkriterien verbergen", 0.6, DisplayType.CHECK, true);
		psd.add(pd);
	...
	private PreferenceTabDescriptor buildConsoleTab() {
		...
		ptd.add(psd);
		pd = new PreferenceDescriptor("maxCharacter", "Max. Zeichen", 0.1, DisplayType.INTEGER, 24000);
		psd.add(pd);
