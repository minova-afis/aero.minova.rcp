...
public class ApplicationPreferenceWindowHandler {
	...
	public void execute(IThemeEngine themeEngine, IWorkbench workbench) {
		...
		if (!currentTheme.equals(newTheme) || !curentSelectAllControls == newSelectAllControls) {
			...
			boolean openConfirm = MessageDialog.openConfirm(activeShell, "Neustart", "Zum übernehmen der Änderungen, muss die Applikation neu gestarted werden");
			if (openConfirm) {
	...
	public PWWidget createWidgets(PWTab tab, PreferenceDescriptor pref, String key, @Optional TranslationService translationService, Object... values) {
		...
		case FONT:
			...
			break;
		case LOCALE:
			widget = new PWLocale(pref.getLabel(), ApplicationPreferences.LOCALE_LANGUAGE, application.getContext(), translationService)
