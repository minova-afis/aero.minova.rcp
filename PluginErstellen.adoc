...
[source,java]
----
MField keyLong = detail.getField("KeyLong");
MButton printInvoice = detail.getButton("PrintInvoice");
printInvoice.setEnabled(keyLong.getValue() != null);
----

Außerdem muss ein Helper auf das Drücken eines Buttons reagieren können. 
Dafür kann ein `SelectionListener` auf den Button registriert werden:

[source,java]
----
MButton printInvoice = detail.getButton("PrintInvoice");
printInvoice.addSelectionListener(new SelectionListener() {
	@Override
...
** SuperUser Modus vorerst durch Einstellung "Experten Modus" ersetzt (für ein-/ausblenden von Feldern/Sections)
